...
Jarvis 提供 **AI 全市场扫描推荐** 功能：自动扫描全 A 股市场（通常 5000+ 只），通过三层筛选流程，最终给出 **值得买入的股票**（0–5 只），附带 AI 评分、推荐理由、风险提示和建议买入价位区间。
...
| 层级 | 名称 | 说明 |
|------|------|------|
| **Layer 1** | 全市场快速筛选 | 从 ~5000 只 A 股中按条件筛选：非ST、涨跌幅 -5%~+8%、换手率 ≥0.5%、成交额 ≥3000万、0 < 动态PE < 80。PE 越低得分越高。取 **前100只** 候选。 |
| **Layer 2** | 详细批量分析 | 对候选逐一进行 **基本面评分**（ROE/成长/估值/负债）、技术分析、情绪分析。RSI>75 的超买股被标记。综合得分 = 基本面35% + 技术25% + 情绪15% + L1得分15% + 估值10%。取 **前30只**。 |
| **Layer 3** | LLM 买入判断 | 使用本地 LLM（Ollama）对候选逐一判断"**现在是否值得买入**"。RSI 超买股自动排除。LLM 必须给出明确 verdict（"买入"或"不买入"），只有 verdict="买入" 且综合分≥60 的才通过。结果 **0–5 只**。 |
...
**Web UI**：点击工具栏 **「🌟 AI推荐」** 按钮打开扫描面板，点击 **「开始扫描」** 启动后台扫描。进度条会实时更新各阶段状态。扫描完成后：

- **有推荐时**：显示推荐卡片，每张包含：
  - 综合得分（0-100）+ 基本面得分
  - 最新价、涨跌幅、PE、技术/情绪得分、RSI
  - 建议买入区间（由 LLM 给出）
  - 推荐理由和主要风险
  - **「⭐ 自选」按钮** — 一键加入自选股列表
- **无推荐时**：显示"暂无推荐"说明，提示这是正常的
...
- 全市场扫描需要网络连接（akshare 数据源），Layer 2 需要 akshare 财务数据，Layer 3 需要 Ollama 运行
- 完整扫描通常需要 15–30 分钟（Layer 2 新增基本面分析增加了时间）
- 扫描中途可随时停止，下次可从断点继续
- **akshare 偶尔连接失败**（`RemoteDisconnected`），这是东方财富服务端限流导致，不影响分析结果（有 fallback API）
- 结果为 0 只是正常的 — 大部分交易日真正值得买入的标的并不多
