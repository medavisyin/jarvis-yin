...
- **快照对比模式**: 系统在你每次点击"获取最新数据"时拍一次快照并保存。第一次点击只有1个快照，无法对比，所以显示"无数据"。**你需要隔几天点一次**来积累数据。
- 建议操作频率：每 **1-3个交易日** 点一次，一周后即可看到变化趋势。
...
→ { "snapshot": { "etf_snapshot": [...], "total_broad_shares_yi": ..., "signals": {...} }, "trend": {...} }
```
