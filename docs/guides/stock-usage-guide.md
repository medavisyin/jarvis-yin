...
勾选扫描旁的 "DeepSeek" 复选框后，TOP 5 推荐股票会额外使用 `deepseek-reasoner` 进行深度分析：

- Layer 3 的 30 只候选仍使用本地 Ollama（节省 API 调用）
- 只有最终推荐的 TOP 5 才调用 DeepSeek API
- 每只股票的综合分析数据（技术面/ML/价格预测/资金流向/Scanner 判定）全部发送给 DeepSeek
- 结果显示在每只股票下方的蓝色 DeepSeek 卡片中，含可展开的推理过程
