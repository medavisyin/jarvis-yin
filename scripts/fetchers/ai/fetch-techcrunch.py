"""
Fetch recent AI articles from TechCrunch.

TechCrunch uses dynamic loading — Playwright waits for content to render
before extracting article cards.

Usage: python fetch-techcrunch.py [output-dir]
Output: <output-dir>/techcrunch.json
"""
import asyncio
import json
import os
...
async def fetch():
    ...
    async with async_playwright() as p:
        ...
        article_els = await page.query_selector_all("article, [class*='post-block']")
        if not article_els:
            article_els = await page.query_selector_all("h2 a, h3 a")

        for el in article_els[:MAX_ITEMS]:
            try:
                ...
                excerpt_el = await el.query_selector("[class*='excerpt'], [class*='summary'], p")
                excerpt = (await excerpt_el.inner_text()).strip()[:300] if excerpt_el else None

                posts.append({"title": title, "url": url, "date": date_text, "excerpt": excerpt})
            except Exception:
                continue
