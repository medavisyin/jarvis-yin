"""
Fetch top world news from Deutsche Welle (DW) via RSS + Playwright drill-down.

DW provides reliable RSS feeds in English. Uses feedparser for headlines
and Playwright for full article drill-down on top stories.

Usage: python fetch-dw-news.py [output-dir]
Output: <output-dir>/dw-news.json
"""
import asyncio
import json
import os
...
RSS_FEEDS = [
    ("world", "https://rss.dw.com/xml/rss-en-world"),
    ("business", "https://rss.dw.com/xml/rss-en-bus"),
    ("science", "https://rss.dw.com/xml/rss-en-sci"),
    ("europe", "https://rss.dw.com/xml/rss-en-eu"),
]
...
def _download_rss(url, timeout=15):
    """Download RSS XML via httpx (supports SOCKS proxy), parse with feedparser."""
    import httpx
...
def _fetch_rss(timing):
    """Fetch and parse all DW RSS feeds."""
    t = time.monotonic()
    seen_titles = set()
    items = []
...
async def _drill_down(page, items, timing):
    """Use an existing Playwright page to fetch full article content for top stories."""
    ...
    for idx, item in enumerate(drill_items):
        ...
        try:
            ...
            paragraphs = await page.query_selector_all(
                "article p, .rich-text p, [class*='RichText'] p, "
                ".longText p, [class*='article-body'] p"
            )
            text_parts = []
...
async def fetch():
    ...
    async with async_playwright() as p:
        proxy_arg = await get_proxy_for_playwright(p, "https://www.dw.com")
