"""
Fetch top world news from BBC via RSS + Playwright drill-down.

Uses BBC World News RSS feed for headlines, then Playwright for
full article content on top stories.

Usage: python fetch-bbc-news.py [output-dir]
Output: <output-dir>/bbc-news.json
"""
import asyncio
import json
import os
...
def _fetch_rss(timing):
    """Fetch and parse all RSS feeds, return deduplicated items."""
    t = time.monotonic()
    seen_titles = set()
    items = []
    ...
    for idx, item in enumerate(drill_items):
        ...
        try:
            ...
            paragraphs = await page.query_selector_all(
                "article p, [data-component='text-block'] p, .ssrcss-pv1fh6-RichTextComponentWrapper p"
            )
            text_parts = []
...
async def fetch():
    ...
    async with async_playwright() as p:
        proxy_arg = await get_proxy_for_playwright(p, "https://www.bbc.com")
